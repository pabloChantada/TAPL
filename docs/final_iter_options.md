...
1. **Control de número de preguntas en el UI**
   - Ahora `TOTAL_QUESTIONS = 2` está fijo en backend, y el frontend también asume 2.
   - Mejoras:
     - Selector en la pantalla inicial: slider o dropdown para elegir 1–10 preguntas.
     - Enviar ese valor a `/api/interview/start` y usarlo en backend (en vez de la constante global).
...
4. **Mejoras de UI/UX**
   ...
   - Indicador visual más claro cuando Gemini está generando pregunta/respuesta.
   - Textos de ayuda (“Qué es BLEU”, “Qué significa ROUGE‑L”, etc.) en tooltips o modales.
...
6. **Evaluación cualitativa con LLM**
   - Usar Gemini también para:
     - Generar un pequeño feedback por respuesta: puntos fuertes, aspectos a mejorar.
     - Generar un resumen final: “En general tu nivel en el dataset X es…”.
